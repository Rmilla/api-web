...
class TestFastAPIRoutes(unittest.TestCase):
    ...
    def test_delete_client(self):
        ...
        response = self.client.delete("/delete_client/43")
    ...
    def test_update_client(self):
        ...
        response = self.client.put(
            "/update_client/45", json=client_data_update)
        self.assertEqual(response.status_code, 200)
        response_data = response.json()
        self.assertEqual(response_data['nom_client'], client_data_update['nom_client'])
        self.assertEqual(response_data['prenom_client'], client_data_update['prenom_client'])
        self.assertEqual(response_data['email_client'], client_data_update['email_client'])
        self.assertEqual(response_data['telephone_client'], client_data_update['telephone_client'])
        self.assertEqual(response_data['preferences_client'], client_data_update['preferences_client'])
        self.assertEqual(response_data['adresse_livraison_client'], client_data_update['adresse_livraison_client'])
        self.assertEqual(response_data['adresse_facturation_client'], client_data_update['adresse_facturation_client'])
