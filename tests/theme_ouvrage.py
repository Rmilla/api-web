...
class TestThemeRoutes(unittest.TestCase):
    ...
    def test_create_theme(self):
        new_theme_data = {"theme_id": 1,"nom_theme": "Fantasy"}
        response = self.client.post("/create_theme", json=new_theme_data)
        self.assertEqual(response.status_code, 201)
        response_data = response.json()
        self.assertEqual(response_data["theme_id"],
                         new_theme_data["theme_id"])
        self.assertEqual(response_data["nom_theme"],
                         new_theme_data["nom_theme"])
