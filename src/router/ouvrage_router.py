...
from ..schema import OuvrageCreate, OuvrageUpdate, OuvrageResponse

router = APIRouter()


@router.post("/models/ouvrage/", response_model=OuvrageResponse, status_code=201, tags=["ouvrage"])
...
@router.get("/models/ouvrage/", tags=["ouvrage"])
...
@router.get("/models/ouvrage/", response_model=List[OuvrageResponse], tags=["ouvrage"])
async def read_ouvrages(titre_ouvrage: str = "", isbn_ouvrage: str = "", theme: str = "", keyword: str = "", BDD_session: Session = Depends(get_db)):
    ...
    if titre_ouvrage:
        ouvrages_query = ouvrages_query.filter(
            Ouvrage.titre_ouvrage.ilike(f"%{titre_ouvrage}%"))
    if isbn_ouvrage:
        ouvrages_query = ouvrages_query.filter(
            Ouvrage.isbn_ouvrage.ilike(f"%{isbn_ouvrage}%"))
    if keyword:
        ouvrages_query = ouvrages_query.filter(
            Ouvrage.mot_cle_ouvrage.ilike(f"%{keyword}%"))
    else:
        raise HTTPException(
            status_code=404, detail="Ce livre n'est pas dans la librairie")

# Mise a jour d'un objet


@router.patch("/models/ouvrage/{ouvrage_id}", response_model=OuvrageResponse, tags=["ouvrage"])
async def update_ouvrage(ouvrage_id: int, ouvrage_update: OuvrageUpdate, BDD_session: Session = Depends(get_db)):
    existing_ouvrage = BDD_session.query(Ouvrage).filter(
        Ouvrage.id_ouvrage == ouvrage_id).first()
...
@router.delete("/models/ouvrage/{id_ouvrage}", response_model=None, status_code=status.HTTP_200_OK, tags=["ouvrage"])
